...
class DataXceiver implements Runnable, FSConstants {
   ...
   * @throws IOException
   */
  private void readBlock(DataInputStream in,DataNode dataNode) throws IOException {
    //
    // Read in the header
    //
    ...
    //PROJECT
    //block.incrementActiveConnection();
    if(dataNode.getConnectionsMap().containsKey(blockId)){
    int value = dataNode.getConnectionsMap().get(blockId);
    dataNode.getConnectionsMap().put(blockId,value+1);
    }
    else
     dataNode.getConnectionsMap().put(blockId,1);
    //System.out.println("The block connection incremented to"+block.getActiveConnection());
    ...
    try {
      ...
      // block.decrementActiveConnection();
      //System.out.println("The number of active connections decremented to"+block.getActiveConnection());
      if (blockSender.isBlockReadFully()) {
        //PROJECT
	int decrement = dataNode.getConnectionsMap().get(blockId);
    	dataNode.getConnectionsMap().put(blockId,decrement-1);
...
System.out.println("In copy block!!!!");
 long blockId = in.readLong(); // read block id
    ...
    try {
      ...
      reply = new DataOutputStream(new BufferedOutputStream(
       ...
       System.out.println("The number of active connections incremented to"+block.getActiveConnection());
      // send status first
